    ...
    try {
      ...
      for (Map<String, String> position : positionsList) {
        Job newJob = new Job(position.get("id"), position.get("type"), position.get("url"),
            position.get("createdAt"), position.get("company"), position.get("companyUrl"),
            position.get("location"),
            position.get("title"), position.get("description"), position.get("howToApply"),
            position.get("companyLogo"));
        jobList.add(newJob);
      }
...
//            System.out.println("test");
  ...
  private static void calculateJobPostingsPerCompany(List<Job> jobsList) {
    ...
    for (Map.Entry<String, Integer> displayJob : jobPostingsMap.entrySet()) {
      System.out
          .println("Company: " + displayJob.getKey() + " Num postings: " + displayJob.getValue());
    }
  }
